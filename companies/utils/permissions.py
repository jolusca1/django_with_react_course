...
def check_permission(user, method, permission_to) -> bool:
    ...
    for group in groups:
        permissions = GroupePermission.objects.values('permission_id').filter(group_id=group['group_id']).all()
...
class EmployeesPermission(permissions.BasePermission):
    message = 'O usuário não tem permissão para gerenciar os funcionário'
...
class TaskPermission(permissions.BasePermission):
    message = 'O usuário não tem permissão para criar ou gerenciar as tarefas de todos os funcionários'
